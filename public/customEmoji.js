...
function closeEmoji(){   //이모티콘 편집 창 닫기
  ...
  $("#spantxt").html('');
  $("#custom_emoji").css("display", "inline-block");
  $("#date").css("top", "90%");
...
function show_sampletxt(pick_emoji){
  ...
  lo_date = pick_emoji["date"];
  lo_name = pick_emoji["username"];
...
//이름 숨기기&보여주기
if($("input:checkbox[name=namechk]").is(":checked") == true) {
  $("#name").css("display", "inline-block");
}
else if ($("input:checkbox[name=namechk]").is(":checked") == false) 
  $("#name").css("display", "none");
//날짜 숨기기&보여주기
if($("input:checkbox[name=datechk]").is(":checked") == true) {
  $("#date").css("display", "inline-block");
}
else if ($("input:checkbox[name=datechk]").is(":checked") == false)
  $("#date").css("display", "none");
}
...
$(function(){          
  $("#save").click(function() {
      ...
      var spndate = $("#spandate").width();
      if(tenchk == 1) {
        if(lo_ten == '상' || lo_ten == '하') {
          imgheight = 120 + spnheight + 30;
          imgwidth = Math.max(120, spnwidth) + spndate;
        }
        else if(lo_ten == '좌' || lo_ten == '우'){
          imgheight = Math.max(120, spnheight)+10;
          imgwidth = 120 + spnwidth  + spndate;
        }
      }
...
function cropImage(image, croppingCoords, sw,sh) {
  ...
  ctx.drawImage(image,-Math.floor(cc.x)+cutx, -Math.floor(cc.y)+cuty); 
