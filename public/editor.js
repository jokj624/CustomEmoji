function setThumbnail(event) {
 ...
 function dateandname(event) {
     var a2 = document.getElementById("a2");
     a2.innerHTML ="";
     var array = ["사용자 이름", "날짜", "미리보기"];
     const lo_array = ["선택","상", "하", "좌", "우"];
     ...
     const date_select = document.createElement("select");

     div.setAttribute("id", "listD");
     ul.setAttribute("id", "newul");
     name_select.setAttribute("id", "name_lo");
     date_select.setAttribute("id", "date_lo");
     name_select.setAttribute("onChange", "changeNameAndDateLocation('name')");
     date_select.setAttribute("onChange", "changeNameAndDateLocation('date')");

     div.innerHTML = "Select Option";
     a2.appendChild(div);
     a2.appendChild(ul);

     name_select.style.display = "none";
    ...
    for (var i=0; i<array.length; i++){
         var li=document.createElement('li');
         if(i==0)   li.setAttribute("id", i);
         if(i==1)   li.setAttribute("id", i);
         if(i==2)   li.setAttribute("id", i);
         ul.appendChild(li);
         ...
         if(i == 0) {
            ...
            ul.insertBefore(name_select, two);
            $("#0").click(function() {  
                if($("#name_lo").is(":visible")){
                    $("#name_lo").slideUp();
                }
                else{	
                    $("#name_lo").slideDown();
         ...
         if(i == 1) {
            ul.appendChild(date_select);
            $("#1").click(function() {  
                if($("#date_lo").is(":visible")){
                    $("#date_lo").slideUp();
                }
                else{	
                    $("#date_lo").slideDown();
...
function changeNameAndDateLocation(choice) {
    let selected, selected_val, t, mt, l, ml, r, mr;
    console.log(choice);
    if(choice == "name"){
        selected = document.getElementById("name_div");
        selected_val = $("#name_lo option:selected").val();
        username = selected_val;
    } else if(choice == "date"){
        selected = document.getElementById("date_div");
        selected_val = $("#date_lo option:selected").val();
        date = selected_val;
    }
    if(selected_val == '상'){
        t = '15%';
    ...
    selected.style.marginTop = mt;
    selected.style.left = l;
    selected.style.marginLeft = ml;
    selected.style.right = r;
    selected.style.marginRight = mr;
 ...
 function saveEmoji(event){	//Emoji name 매개변수로 받아와야함
     ...
     if(document.getElementById('textarea10')){  //10자 사용 할 때
         ...
         saveInfo[index].ten.location = "상";  //10자 사용 안할 때
     }
     if(document.getElementById('textarea')){	//30자 사용 할 때
         ...
         saveInfo[index].thirty.location = "우";	//30자 사용 안할 때
     }
     saveInfo[index].username = username;
     saveInfo[index].date = date;
 ...
 var username = "하";
 var date = "하";		        //기본으로 추가하고 안 움직였을 때 위치
...
function sample10Preview(){
    ...
    if(username == '상'){
        ...
        $('#nameprev').css('top', '');
        $('#nameprev').css('left', '');
        $('#nameprev').css('bottom', '43%');
    } else if(username == '좌'){
        $('#nameprev').css('left', '45%');
        $('#nameprev').css('bottom', '');
        $('#nameprev').css('top', '25%');
    } else if(username == '우'){
        $('#nameprev').css('bottom', '');
        $('#nameprev').css('top', '25%');
        $('#nameprev').css('left', '85%');
    }
    //날짜 미리보기
    if(date == '상'){
        $('#dateprev').css('top', '0%');
        $('#dateprev').css('left','70%');
    } else if(date == '하'){
        $('#dateprev').css('top', '');
        $('#dateprev').css('left', '68%');
        $('#dateprev').css('bottom', '43%');
    } else if(date == '좌') {
        $('#dateprev').css('top', '25%');
        $('#dateprev').css('bottom', '');
        $('#dateprev').css('left', '35%');
    } else if(date == '우') {
        $('#dateprev').css('top', '25%');
        $('#dateprev').css('bottom', '');
        $('#dateprev').css('left', '80%');
    }
}
